...
test.describe('History administration', () => {
  ...
  test('delete a history entry', async ({ apiClient, page, entityFormPage }) => {
    ...
    const rows = page.locator('tr', { hasText: String(TEST_DATA.history.year) })
    const countBefore = await rows.count()
    await rows.first().locator('a[href*="history/delete"]').click()
    await entityFormPage.submit()
    await expect(page).toHaveURL(/\/history/)
    await expect(page.locator('tr', { hasText: String(TEST_DATA.history.year) })).toHaveCount(countBefore - 1)
  })
})
