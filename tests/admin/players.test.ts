...
test.describe('Player administration', () => {
  ...
  test('create a new player', async ({ entityFormPage, page }) => {
    ...
    await expect(page.locator('body')).toContainText(`${TEST_DATA.player.lastName}, ${TEST_DATA.player.firstName}`)
  })

  test('edit a player', async ({ apiClient, page, entityFormPage }) => {
    await apiClient.createPlayer()
    await page.goto('/league/players')
    const row = page.locator('tr', { hasText: `${TEST_DATA.player.lastName}, ${TEST_DATA.player.firstName}` }).first()
  ...
  test('delete a player', async ({ apiClient, page, entityFormPage }) => {
    await apiClient.createPlayer()
    await page.goto('/league/players')
    const rows = page.locator('tr', { hasText: `${TEST_DATA.player.lastName}, ${TEST_DATA.player.firstName}` })
    const countBefore = await rows.count()
    await rows.first().locator('a[href*="player/delete"]').click()
    await entityFormPage.submit()
    await expect(page).toHaveURL(/\/league\/players/)
    await expect(page.locator('tr', { hasText: `${TEST_DATA.player.lastName}, ${TEST_DATA.player.firstName}` })).toHaveCount(countBefore - 1)
  })
})
